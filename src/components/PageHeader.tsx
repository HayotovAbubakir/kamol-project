...
export function PageHeader({ title, description, subtitle, actions, compact = false }: PageHeaderProps) {
  return (
    <div className={cn('flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-4', compact ? 'mb-2 sm:mb-3 tv:mb-4' : 'mb-5 sm:mb-6 md:mb-8')}>
      <div className="min-w-0">
        ...
        <h1
          className={cn(
            'font-display font-bold tracking-tight text-app-text',
            compact
              ? 'text-lg sm:text-xl md:text-2xl tv:text-3xl tv:md:text-4xl'
              : 'text-2xl sm:text-3xl md:text-4xl lg:text-[2.75rem] tv:text-5xl',
          )}
        >
          {title}
        </h1>
