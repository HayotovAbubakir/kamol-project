  ...
  const content = (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <p className={cn('font-semibold uppercase text-app-muted', compact ? 'text-[11px] tracking-wide' : 'text-xs tracking-[0.16em]')}>
          {label}
        </p>
        {icon && (
          <span className={cn('flex items-center justify-center rounded-xl bg-app-accent/10 text-app-accent', compact ? 'h-7 w-7' : 'h-9 w-9')}>
            {icon}
          </span>
        )}
      </div>
      <p className={cn('font-display font-bold text-app-accent', compact ? 'mt-1 text-xl' : 'mt-4 text-4xl')}>{value}</p>
